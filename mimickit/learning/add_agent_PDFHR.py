...
class ADDAgent(amp_agent_PDFHR.AMPAgent):
    ...
    def _build_normalizers(self):
        super(amp_agent_PDFHR.AMPAgent, self)._build_normalizers()
    ...
    def _record_data_post_step(self, next_obs, r, done, next_info):
        super(amp_agent_PDFHR.AMPAgent, self)._record_data_post_step(next_obs, r, done, next_info)
        ...
        if self._env.use_PDFHR_reward:
            self._exp_buffer.record("PDFHR_reward", self._env._PDFHR_reward_buf)
        return
    
    def _build_train_data(self):
        reward_info = self._compute_rewards()
        info = super(amp_agent_PDFHR.AMPAgent, self)._build_train_data()
    ...
    def _compute_rewards(self):
        ...
        if self._env.use_PDFHR_reward:
            PDFHR_dist_r = self._exp_buffer.get_data_flat("PDFHR_reward")
            PDFHR_dist_reward_std, PDFHR_dist_reward_mean = torch.std_mean(PDFHR_dist_r)
            r = r + PDFHR_dist_r
            info['PDFHR_dist_reward_mean'] = PDFHR_dist_reward_mean
            info['PDFHR_dist_reward_std'] = PDFHR_dist_reward_std
