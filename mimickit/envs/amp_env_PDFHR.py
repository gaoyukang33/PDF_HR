...
class AMPEnv(deepmimic_env_PDFHR.DeepMimicEnv):
    ...
    def _build_data_buffers(self):
        ...
        if self.use_PDFHR_reward:
            ...
            self._PDFHR_reward_buf = torch.zeros(num_envs, device=self._device, dtype=torch.float)
        return
    ...
    def _update_done(self):
        ...
        self._done_buf[:] = deepmimic_env_PDFHR.compute_done(done_buf=self._done_buf,
                                         time=self._time_buf, 
    ...
    def _update_reward(self):
        ...
        if enable_PDFHR_reward:
            char_id = self._get_char_id()
            dof_pos = self._engine.get_dof_pos(char_id)
            assert(self._reward_PDFHR_mode == "static"), "AMP only support static PDFHR rewards"
            ...
            self._PDFHR_reward_buf[:] = PDFHR_dist_r
        return
...
def compute_disc_obs(ref_root_pos, ref_root_rot, root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_vel, key_pos, global_obs, root_height_obs):
    ...
    pos_obs = deepmimic_env_PDFHR.compute_tar_obs(ref_root_pos=ref_root_pos, 
                                               ref_root_rot=ref_root_rot, 
                                               root_pos=root_pos, 
                                               root_rot=root_rot, 
                                               joint_rot=joint_rot, 
