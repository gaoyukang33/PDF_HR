...
class AMPEnv(deepmimic_env_nrdf.DeepMimicEnv):
    ...
    def _build_data_buffers(self):
        ...
        if self.use_nrdf_reward:
            ...
            self._nrdf_reward_buf = torch.zeros(num_envs, device=self._device, dtype=torch.float)
        return
    ...
    def _update_done(self):
        ...
        self._done_buf[:] = deepmimic_env_nrdf.compute_done(done_buf=self._done_buf,
                                         time=self._time_buf, 
...
def compute_disc_obs(ref_root_pos, ref_root_rot, root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_vel, key_pos, global_obs, root_height_obs):
    ...
    pos_obs = deepmimic_env_nrdf.compute_tar_obs(ref_root_pos=ref_root_pos, 
                                               ref_root_rot=ref_root_rot, 
                                               root_pos=root_pos, 
                                               root_rot=root_rot, 
                                               joint_rot=joint_rot, 
