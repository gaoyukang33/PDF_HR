...
from learning.nets.nrdf_net import PoseDistanceMLP
from learning.nets.nrdf_net_pp import NRDF_Adapter
...
class DeepMimicEnv(char_env.CharEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        self.use_nrdf_reward = env_config.get("use_nrdf_reward")
        self.use_nrdf_early_termination = env_config.get("use_nrdf_early_termination")
        self.use_nrdf_regularization = env_config.get("use_nrdf_regularization")
        if self.use_nrdf_reward or self.use_nrdf_early_termination or self.use_nrdf_regularization:
            # self.nrdf_model = PoseDistanceMLP(hidden=(1024, 512, 256, 128), dropout=0.1).to(device)
            # state_dict = torch.load('/home/ubuntu/code/final_exp/MimicKit/prior_ckpt/pose_dist_L1_50.pt')['model']
            
            self.nrdf_model = NRDF_Adapter(device=device)
            state_dict = torch.load('/home/ubuntu/code/final_exp/MimicKit/prior_ckpt/nrdf_epoch_70.pt')['model']
            
            self._motion_file = env_config.get("motion_file")
            # Load the state_dict into the model's parameters
            self.nrdf_model.load_state_dict(state_dict)
            self.nrdf_model.train(False)
            with open(self._motion_file, 'rb') as filestream:
                in_dict = pickle.load(filestream)
                frames = np.array(in_dict['frames'], dtype=np.float32)[:, 6:]
            q = torch.from_numpy(frames).to(device)
            tolerance = 0.05
            self.nrdf_max_ref_motion_dist = self.nrdf_model(q).max() 
            if self.nrdf_max_ref_motion_dist < 0.2:
                self.nrdf_max_ref_motion_dist = self.nrdf_max_ref_motion_dist + tolerance

            if self.use_nrdf_early_termination:
                self.nrdf_early_termination_mode = env_config.get("nrdf_early_termination_mode")
                self.nrdf_et_counter = 0
                self.nrdf_max = 0
                with torch.no_grad():
                    if self.nrdf_early_termination_mode == 'etcurr':
                        self.etcurr_lower = env_config.get("etcurr_lower")
                        self.etcurr_upper = env_config.get("etcurr_upper")
                        self.etcurr_range = abs(self.etcurr_upper - self.etcurr_lower)
                        self._nrdf_early_termination_threshold = self.nrdf_max_ref_motion_dist + self.etcurr_lower  # update before compute_done
                    elif self.nrdf_early_termination_mode == 'etth':
                        self.etth_range = env_config.get("etth_range")
                        self._nrdf_early_termination_threshold = self.nrdf_max_ref_motion_dist + self.etth_range
                    elif self.nrdf_early_termination_mode == 'et':
                        self.et_range = env_config.get("et_range")
                        self._nrdf_early_termination_threshold = self.et_range
                    else:
                        print(f'early termination mode {self.nrdf_early_termination_mode} is not supported')
                        exit()
                    # print(self._nrdf_early_termination_threshold)
            if self.use_nrdf_reward:
                self._reward_nrdf_dist_w = env_config.get("reward_nrdf_dist_w")
                self._reward_nrdf_dist_scale = env_config.get("reward_nrdf_dist_scale")
                self._reward_th = env_config.get("reward_th")
            if self.use_nrdf_regularization:
                self.nrdf_regularization_loss_weight = env_config.get("nrdf_regularization_loss_weight")
                self.nrdf_regularization_threshold = env_config.get("nrdf_regularization_threshold")
            
            self._max_test_mean_time = 0.0
            self._max_train_mean_time = 0.0
            self.nrdf_reward_flag = True   
    ...
    def _update_reward(self):
        ...
        # enable_nrdf_reward = self.use_nrdf_reward
        if enable_nrdf_reward:
            # print('enable_nrdf_reward')
            with torch.no_grad():
                nrdf_pred = self.nrdf_model(dof_pos).view(-1)
                # d_good = self.nrdf_max_ref_motion_dist
                # d_bad = min(3 * d_good, 0.4)

                d_bad = 0.4
                d_good = self.nrdf_max_ref_motion_dist
                # d_good = max(0.1, self.nrdf_max_ref_motion_dist)

                s_d = (nrdf_pred - d_good) / (d_bad - d_good)
                s_d = torch.clamp(s_d, 0, 1)
                # nrdf_dist_r = self._reward_nrdf_dist_w * (1 - s_d)
                nrdf_dist_r = self._reward_nrdf_dist_w * torch.exp(-s_d)   # [0, 1]
        ...
        if enable_nrdf_reward:
            print("nrdf: ", nrdf_dist_r.mean())
            print("buf: ", self._reward_buf.mean())
    ...
    def _update_done(self):
        ...
        if enable_nrdf_termination:
            ...
            # if self.nrdf_early_termination_mode == 'etcurr':
            #     self._nrdf_early_termination_threshold = self.nrdf_max_ref_motion_dist + self.etcurr_lower + self.etcurr_range * (1 - self._curr_max_test_mean_ep_len / 300.)
            # dof_nrdf_overflow = self.nrdf_model(dof_pos) - self._nrdf_early_termination_threshold

            with torch.no_grad():
                nrdf_pred = self.nrdf_model(dof_pos).view(-1)
                if nrdf_pred.max() > self.nrdf_max:
                    self.nrdf_max = nrdf_pred.max()
                d_good = self.nrdf_max_ref_motion_dist
                d_bad = 0.5
                s_d = (nrdf_pred - d_good) / (d_bad - d_good)  # [0, 1]
                s_d = torch.clamp(s_d, 0, 1)

            dof_nrdf_overflow = s_d - 0.5
            self.nrdf_et_counter += (dof_nrdf_overflow > 0).to(torch.int32).sum()
            # print("nrdf triggered: ", self.nrdf_et_counter)
            print("nrdf max", self.nrdf_max)
            # print(dof_nrdf_overflow.max())
            # print("early triggered")
        if self._mode == base_env.EnvMode.TEST:
            self._max_test_mean_time = max(self._max_test_mean_time, self._time_buf.mean())
        #     # print("max_test_mean_time: ", self._max_test_mean_time)
        if self._mode == base_env.EnvMode.TRAIN:
            self._max_train_mean_time = max(self._max_train_mean_time, self._time_buf.mean())
            # print("_max_train_mean_time: ", self._max_train_mean_time)
