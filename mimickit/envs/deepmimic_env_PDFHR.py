...
from learning.nets.PDFHR_net import PoseDistanceMLP
from learning.nets.PDFHR_net_pp import PDFHR_Adapter
...
class DeepMimicEnv(char_env.CharEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        # PDFHR Init
        # use_PDFHR_reward, use_PDFHR_early_termination, PDFHR_et_threshold, PDFHR_model_path, reward_PDFHR_dist_w, reward_PDFHR_dist_scale
        self.use_PDFHR_reward = env_config.get("use_PDFHR_reward")
        self.use_PDFHR_early_termination = env_config.get("use_PDFHR_early_termination")
        if self.use_PDFHR_early_termination or self.use_PDFHR_reward:
            self.PDFHR_model = PDFHR_Adapter(device=device)
            state_dict = torch.load(env_config.get("PDFHR_model_path"))['model']
            self.PDFHR_model.load_state_dict(state_dict)
            self.PDFHR_model.train(False)

        if self.use_PDFHR_early_termination:
            self.PDFHR_et_counter = 0
            self.PDFHR_max = 0
            self.PDFHR_et_threshold = env_config.get("PDFHR_et_threshold")

        if self.use_PDFHR_reward:
            self._reward_PDFHR_dist_w = env_config.get("reward_PDFHR_dist_w")
            self._reward_PDFHR_dist_scale = env_config.get("reward_PDFHR_dist_scale")
            self._reward_PDFHR_tolerance = env_config.get("reward_PDFHR_tolerance")
            self._reward_PDFHR_mode = env_config.get("reward_PDFHR_mode")
    ...
    def _load_motions(self, motion_file):
        ...
        if self.use_PDFHR_reward:
            if self._reward_PDFHR_mode == "static":
                q = self._motion_lib.joint_rot_to_dof(self._motion_lib._frame_joint_rot)

                chunk_size = 1024
                self.PDFHR_max_ref_motion_dist = 0
                for i in range(0, q.shape[0], chunk_size):
                    cur_dis_max = self.PDFHR_model(q[i:i+chunk_size]).max() 
                    if cur_dis_max > self.PDFHR_max_ref_motion_dist:
                        self.PDFHR_max_ref_motion_dist = cur_dis_max
        return
    ...
    def _update_reward(self):
        ...
        # PDFHR reward trigger, only used in training time                                             
        enable_PDFHR_reward = (self._mode == base_env.EnvMode.TRAIN) and self.use_PDFHR_reward
        if enable_PDFHR_reward:
            # print('enable_PDFHR_reward')
            with torch.no_grad():
                PDFHR_pred = self.PDFHR_model(dof_pos).view(-1)
                if self._reward_PDFHR_mode == "static":
                    d_good = min(0.395, self.PDFHR_max_ref_motion_dist + self._reward_PDFHR_tolerance)  # make sure d_good < d_bad
                elif self._reward_PDFHR_mode == "dynamic":
                    ref_dof_pos = self._kin_char_model.rot_to_dof(self._ref_joint_rot)
                    ref_PDFHR = self.PDFHR_model(ref_dof_pos).view(-1)
                    d_good = ref_PDFHR + self._reward_PDFHR_tolerance
                    d_good = torch.clamp(d_good, 0, 0.395)  # # make sure d_good < d_bad
                else:
                    assert(False), "Wrong PDFHR reward mode."

            d_bad = 0.4
            
            s_d = (PDFHR_pred - d_good) / (d_bad - d_good)
            s_d = torch.clamp(s_d, 0, 1)
            PDFHR_dist_r = self._reward_PDFHR_dist_w * torch.exp(-self._reward_PDFHR_dist_scale * s_d)
            self._reward_buf[:] += PDFHR_dist_r
        return
    ...
    def _update_done(self):
        ...
        dof_PDFHR_overflow = torch.zeros_like(body_pos)
        enable_PDFHR_termination = (self._mode == base_env.EnvMode.TRAIN) and self.use_PDFHR_early_termination
        if enable_PDFHR_termination:
            dof_pos = self._engine.get_dof_pos(char_id)

            with torch.no_grad():
                PDFHR_pred = self.PDFHR_model(dof_pos).view(-1)

            dof_PDFHR_overflow = PDFHR_pred - self.PDFHR_et_threshold  # Easy ET
            self.PDFHR_et_counter += (dof_PDFHR_overflow > 0).to(torch.int32).sum()
            print("PDFHR triggered: ", self.PDFHR_et_counter)
        ...
        self._done_buf[:] = compute_done(done_buf=self._done_buf,
                                         ...
                                         track_root=track_root,
                                         dof_PDFHR_overflow=dof_PDFHR_overflow,
                                         enable_PDFHR_termination=enable_PDFHR_termination
                                         )
        return
...
def compute_done(done_buf, time, ep_len, root_rot, body_pos, tar_root_rot, tar_body_pos, 
    ...
    if (enable_early_termination):
        ...
        if enable_PDFHR_termination:
            failed = torch.logical_or(failed, dof_PDFHR_overflow > 0.)
