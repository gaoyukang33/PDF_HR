...
class DeepMimicEnv(char_env.CharEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        # NRDF Init
        # use_nrdf_reward, use_nrdf_early_termination, nrdf_et_threshold, nrdf_model_path, reward_nrdf_dist_w, reward_nrdf_dist_scale
        ...
        if self.use_nrdf_early_termination or self.use_nrdf_reward:
            self.nrdf_model = NRDF_Adapter(device=device)
            state_dict = torch.load(env_config.get("nrdf_model_path"))['model']
        ...
        if self.use_nrdf_early_termination:
            ...
            self.nrdf_max = 0
            self.nrdf_et_threshold = env_config.get("nrdf_et_threshold")
        ...
        if self.use_nrdf_reward:
            ...
            self._reward_nrdf_tolerance = env_config.get("reward_nrdf_tolerance")
            self._reward_nrdf_mode = env_config.get("reward_nrdf_mode")
    ...
    def _load_motions(self, motion_file):
        ...
        if self.use_nrdf_reward:
            if self._reward_nrdf_mode == "static":
                ...
                chunk_size = 1024
                self.nrdf_max_ref_motion_dist = 0
                for i in range(0, q.shape[0], chunk_size):
                    cur_dis_max = self.nrdf_model(q[i:i+chunk_size]).max() 
                    if cur_dis_max > self.nrdf_max_ref_motion_dist:
                        self.nrdf_max_ref_motion_dist = cur_dis_max
        return
    ...
    def _update_reward(self):
        ...
        # nrdf reward trigger, only used in training time                                             
